...
introduce = tk.Button(frame2, relief="solid", text="팀 소개",
                      padx=10, pady=10, command=lambda: [openframe(frame1)])
main = tk.Button(frame1, text="돌아가기", padx=10, pady=10,
...
gender_select = tk.LabelFrame(
    frame2, relief='solid', text="성별을 선택하세요", labelanchor='n')
gender_select.pack(ipady=2)

global gender
gender = tk.StringVar()  # 여기에 int 형으로 값을 저장한다
man = tk.Radiobutton(gender_select, text="남성",
                     variable=gender, value='M', bg="dodgerblue", activebackground="dodgerblue")
woman = tk.Radiobutton(gender_select, text="여성",
                       variable=gender, value='F', bg="red", activebackground="red")
man.pack(padx=10)
woman.pack(padx=10)

mode_select = tk.LabelFrame(frame2, relief='solid',
                            text="모드을 선택하세요", labelanchor='n')
...
func = tk.StringVar()
text = tk.Button(mode_select, text="랜덤 추천 모드", bg="lightcoral",
                 command=lambda: [openframe(frame3)])
Label(frame3, width=15, height=1, text="랜덤 추천 모드", bg="lightcoral").pack(pady=10)
main3 = tk.Button(frame3, text="돌아가기", padx=10, pady=10,
                  command=lambda: [openframe(frame2)])
text.pack(pady=5)
main3.pack()
...
# 음성인식 모드
voice = tk.Button(mode_select, text="랜덤 추천 모드 (음성인식)",
                  bg="darkorange", command=lambda: [openframe(frame4)])
Label(frame4, width=20, height=1, text="랜덤 추천 모드 (음성인식)",
      bg="darkorange").pack(pady=10)
main3 = tk.Button(frame4, text="돌아가기", padx=10, pady=10,
                  command=lambda: [openframe(frame2)])
voice.pack(pady=5)
main3.pack()
...
# 북마크 추천 모드
bookmark = tk.Button(mode_select, text="북마크 추천 모드",
                     bg="khaki", command=bookmark_)
bookmark.pack(pady=5)

# 카테고리 모드
category = tk.Button(mode_select, text="카테고리 선택", bg="lime",
                     command=lambda: [openframe(frame5)])
category.pack(pady=5)
Label(frame5, width=15, height=1, text="카테고리 선택", bg="lime").pack(pady=10)
main3 = tk.Button(frame5, text="돌아가기", padx=10, pady=10,
                  command=lambda: [openframe(frame2)])
main3.pack()
menubutton = tk.Menubutton(frame5, text="음식 종류 선택",
                           relief="raised", bg="thistle", direction="right")
menubutton.pack(pady=10)

a = ["KoreanFood", "ChineseFood", "HealthFood",
     "FastFood"]           # 콤보 박스에 나타낼 항목 리스트
str = StringVar()
combobox = tk.ttk.Combobox(frame5, textvariable=str,
                           width=10)       # root라는 창에 콤보박스 생성
...
# 카메라 인식 모드
camera = tk.Button(mode_select, text="카메라 인식 모드",
                   bg="royalblue", command=camera_)
camera.pack(pady=5)
