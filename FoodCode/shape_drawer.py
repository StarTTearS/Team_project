...
import abc  # 추상 기본 클래스(ABC)를 사용하기 위해 abc 모듈을 가져옵니다.
from typing import (
    Callable,
    Generic,
    List,
...
import pandas as pd  # 엑셀 파일을 사용하기 위해 pandas 모듈을 가져옵니다.
import cv2  # OpenCV를 사용하기 위해 cv2 모듈을 가져옵니다.
import numpy as np  # 배열 작업을 위해 numpy 모듈을 가져옵니다.

from otx.api.entities.annotation import (  # otx API에서 주석과 관련된 엔티티들을 가져옵니다.
    Annotation,
    AnnotationSceneEntity,
    NullAnnotationSceneEntity,
)
from otx.api.entities.coordinate import Coordinate  # otx API에서 좌표 엔티티를 가져옵니다.
from otx.api.entities.label import LabelEntity  # otx API에서 레이블 엔티티를 가져옵니다.
from otx.api.entities.scored_label import ScoredLabel  # otx API에서 점수가 매겨진 레이블 엔티티를 가져옵니다.
from otx.api.entities.shapes.ellipse import Ellipse  # otx API에서 타원 모양을 가져옵니다.
from otx.api.entities.shapes.polygon import Polygon  # otx API에서 다각형 모양을 가져옵니다.
from otx.api.entities.shapes.rectangle import Rectangle  # otx API에서 사각형 모양을 가져옵니다.
from otx.api.entities.shapes.shape import ShapeEntity  # otx API에서 모양 엔티티를 가져옵니다.

CvTextSize = NewType("CvTextSize", Tuple[Tuple[int, int], int])  # OpenCV에서 텍스트 크기를 나타내는 형식을 정의합니다.
excel_data = pd.read_excel('/home/busan/Downloads/Team_project-main/FoodCode/Food_Nutrient.xlsx')  # 엑셀 파일을 읽어옵니다.
_Any = TypeVar("_Any")  # 모든 유형을 나타내는 유니온 유형입니다.
...
class ShapeDrawer(DrawerEntity[AnnotationSceneEntity]):
    ...
    def draw(
        ...
        if not isinstance(entity, NullAnnotationSceneEntity):
                ...
                if (
                    ...
                    for drawer in self.shape_drawers:
                        if type(annotation.shape) in drawer.supported_types and len(annotation.get_labels()) > 0:
                            image = drawer.draw(image, annotation.shape, labels=annotation.get_labels(), frame=image)

                            # 가장 높은 신뢰도를 가진 레이블을 가져옵니다.
                            for label in annotation.get_labels():
                                if isinstance(label, ScoredLabel):
                                    label_name = label.name
                                    confidence = label.probability
                                    # 엑셀 데이터와 일치하는 경우, 일치하는 데이터를 추가합니다.
                                    matched_data = excel_data[excel_data['label'] == label_name]
                                    if not matched_data.empty:
                                        matched_data_list.append((label_name, confidence, matched_data))
            ...
            # 모든 인식된 객체에 대한 객체 정보를 표시합니다.
            for label_name, confidence, matched_data in matched_data_list:
                print(f"인식된 물체: {label_name}, 신뢰도: {confidence}")
                image = self.top_left_drawer.display_object_info_from_xlsx(image, matched_data)

            # 여러 물체의 정보의 총합을 표시합니다.
            image = self.top_left_drawer.display_total_object_info_from_xlsx(image, [matched_data for _, _, matched_data in matched_data_list])
            
            # Capture screen when 'c' key is pressed
            key = cv2.waitKey(1) & 0xFF
            if key == ord('c'):
                cv2.imshow("Captured Image", image)
                save_file = '../Food_img/food.jpg'
                cv2.imwrite(save_file, image)
                cv2.waitKey(0)
                cv2.destroyAllWindows()
                
            elif key == ord('q'):  # 종료를 위한 조건 추가
                cv2.destroyAllWindows()
                exit()
    ...
    class TopLeftDrawer(Helpers, DrawerEntity[Annotation]):
        ...
        def display_object_info_from_xlsx(self, image: np.ndarray, matched_data: pd.DataFrame) -> np.ndarray:
            """Excel 파일에서 일치하는 데이터를 이미지에 표시합니다.

            Args:
                image (np.ndarray): 이미지
                matched_data (pd.DataFrame): 일치하는 데이터가 포함된 DataFrame

            Returns:
                np.ndarray: 데이터가 표시된 이미지
            """
            ...
            text = f"{matched_data['label'].values[0]} Info : calorie = {matched_data['calorie'].values[0]}kcal, carbs = {matched_data['carbs'].values[0]}g, protein = {matched_data['protein'].values[0]}g"
            text2 = f"fat = {matched_data['fat'].values[0]}g, sugar = {matched_data['sugar'].values[0]}g, sodium = {matched_data['sodium'].values[0]}mg"
            color = self.white

            text_scale = self.generate_text_scale(image)
            draw_command_1, _, _ = self.generate_draw_command_for_text(
                text, text_scale, self.top_left_box_thickness, color
            )                        
            text_scale = self.generate_text_scale(image)
            draw_command_2, _, _ = self.generate_draw_command_for_text(
                text2, text_scale, self.top_left_box_thickness, color
            )
            image = draw_command_1(image)
            self.newline()
            image = draw_command_2(image)
            self.newline()

            
            return image

        def display_total_object_info_from_xlsx(self, image: np.ndarray, matched_data_list: List[pd.DataFrame]) -> np.ndarray:
            """Display the total sum of information for all matched objects on the image.

            Args:
                image (np.ndarray): Image.
                matched_data_list (List[pd.DataFrame]): List of DataFrames containing matched data for each object.

            Returns:
                np.ndarray: Image with total sum of information displayed.
            """
            total_calorie = sum(matched_data['calorie'].values[0] for matched_data in matched_data_list)
            total_carbs = sum(matched_data['carbs'].values[0] for matched_data in matched_data_list)
            total_protein = sum(matched_data['protein'].values[0] for matched_data in matched_data_list)
            total_fat = sum(matched_data['fat'].values[0] for matched_data in matched_data_list)
            total_sugar = sum(matched_data['sugar'].values[0] for matched_data in matched_data_list)
            total_sodium = sum(matched_data['sodium'].values[0] for matched_data in matched_data_list)

            total_info_text1 = f"Total Info: calorie = {total_calorie}kcal, carbs = {total_carbs}g, protein = {total_protein}g"
            total_info_text2 = f"fat = {total_fat}g, sugar = {total_sugar}g, sodium = {total_sodium}mg"
            color = self.black

            text_scale = self.generate_text_scale(image)
            draw_command1, _, _ = self.generate_draw_command_for_text(
                total_info_text1, text_scale, self.top_left_box_thickness, color
            )
            text_scale = self.generate_text_scale(image)
            draw_command2, _, _ = self.generate_draw_command_for_text(
                total_info_text2, text_scale, self.top_left_box_thickness, color
            )
            image = draw_command1(image)
            self.newline()
            image = draw_command2(image)
            self.newline()
            return image
    ...
    class RectangleDrawer(Helpers, DrawerEntity[Rectangle]):
        ...
        def draw(self, image: np.ndarray, entity: Rectangle, labels: List[ScoredLabel], frame: np.ndarray) -> np.ndarray:
            ...
            Args:
                ...
                frame (np.ndarray): Frame to display the object information.
