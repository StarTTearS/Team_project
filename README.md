...
```plain
* 세상에는 결정장애를 겪는 사람들이 예상보다 많고, 이들이 시간을 낭비하는 것을 줄여주기 위한 프로그램이 필요하다. 또한, 건강에 대한
관심이 높아지면서 제로콜라 열풍과 같은 트렌드가 나타나고 있어, 개개인이 먹는 음식의 칼로리를 계산하고 건강에 적합한 음식을 추천해주는
서비스가 필요하다. 이러한 요구에 부합하기 위해 주 타겟인 결정장애자부터 다이어트 및 운동에 관심이 있는 20~30대 일반인 등 다양한
사용자를 대상으로 음식 정보 제공 및 랜덤 음식 추천 등의 주요 기능을 갖춘 프로그램을 개발하고자 한다.
```
